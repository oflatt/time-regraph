...
(define (time-suite filename)
  ...
  (define time-file (open-output-file (build-path (current-directory)
                                                  "timing"
                                                  (string-append filename "-total"))))
  (define merge-file (open-output-file (build-path (current-directory)
                                                  "timing"
                                                  (string-append filename "-merge"))))
  (define upwards-file (open-output-file (build-path (current-directory)
                                                  "timing"
                                                  (string-append filename "-upwards"))))
  (define all-regraphs
    (spawn-all-regraphs file-port)))
